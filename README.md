...
**Explicitly left off for now**: the Cabinet web portal, support ticket system, referral withdrawals, admin reports/notifications, SMTP email, and every payment provider except Stars. Each is a real, working feature in Bedolaga - just not something we asked for yet. Turning one on is a config change in `/opt/bedolaga/.env` plus recreating the container (`docker compose up -d --force-recreate bot` - plain `restart` does *not* re-read `.env`, only container creation resolves `env_file`), not a code change.

**Currently in maintenance mode** (`MAINTENANCE_MODE=true`, [`bedolaga/.env.sample`](bedolaga/.env.sample)) while still being tested - blocks everyone except `ADMIN_IDS` with a "check back later" message instead of letting strangers use the bot. Turning this on exposed a real startup race in Bedolaga's own `maintenance_service.py` (the env-driven enable and the Redis-cached-status load can run in either order, and the cache can clobber the freshly-enabled state back off) - worked around by writing the correct state directly into the `maintenance_status` Redis key rather than relying on the env flag alone; documented in `.env.sample` in case it needs redoing later (e.g. after a Redis flush).
